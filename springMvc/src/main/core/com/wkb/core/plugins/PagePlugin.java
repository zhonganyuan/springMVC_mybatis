...
import com.wkb.core.util.ReflectHelper;
...
public class PagePlugin implements Interceptor {
		...
		if(ivk.getTarget() instanceof RoutingStatementHandler){
			...
			if(mappedStatement.getId().matches(pageSqlId)){ //拦截需要分页的SQL
				...
				if(parameterObject==null){
					...
					//update by zhangjm 2016-11-10 15:14:48  效率问题
					String countSql = "" ;
					String sqlTemp = new String(sql);
					sqlTemp = sqlTemp.replaceAll(" ", "").toUpperCase();
					if (sqlTemp.indexOf("GROUPBY") > 0) {
	 ...
	 * @return
	 */
	protected static String getCountSql(String sqlPrimary) {
        ...
        StringBuilder return_sql = new StringBuilder("SELECT count(1)");
